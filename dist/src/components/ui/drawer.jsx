...
const Drawer = ({ shouldScaleBackground = true, ...props }) => (<vaul_1.Drawer.Root direction="right" preventScrollRestoration={false} shouldScaleBackground={shouldScaleBackground} {...props}/>);
...
const DrawerContent = React.forwardRef(({ className, children, ...props }, ref) => (<DrawerPortal>
    <DrawerOverlay />
    <vaul_1.Drawer.Content ref={ref} className={(0, utils_1.cn)("bg-background fixed right-0 top-0 z-50 flex h-full w-[300px] translate-x-full transform flex-col border-l transition-transform", // Ensuring it's on the right and prepared for horizontal animation
    className)} {...props}>
...
const DrawerDescription = React.forwardRef(({ className, ...props }, ref) => (<vaul_1.Drawer.Description ref={ref} className={(0, utils_1.cn)("text-muted-foreground text-sm", className)} {...props}/>));
